...
Notez que si vous avez encore un accès administrateur, il est préférable d'utiliser [la méthode indiquée plus bas](99_detecteur_de_presence_sous_home_assistant.md#graphique) pour modifier le mot de passe à l'aide de l'interface Jeedom.
...
* À l'aide de l'application mobile officielle que vous pouvez acheter directement sur Jeedom Market au coût de 4 € (environ 6 $ CA). Les instructions sont données ici : « application\_mobile\_officielle\_pour\_acceder\_a\_jeedom\_a\_distance ».
...
Si vous ne possédez pas de nom de domaine, vous devrez en réserver un et en assumer les frais. Les instructions sont données ici : « Choisir\_et\_reserver\_son\_nom\_de\_domaine ».
...
Vous devez savoir que cette application n'est pas gratuite. Si vous préférez ne rien débourser, je vous propose une autre technique sur cette fiche : « acceder\_a\_jeedom\_a\_distance\_gratuitement ».

Si vous souhaitez installer l'application Jeedom, dont la configuration est beaucoup plus simple, suivez ces étapes :

* Rendez-vous dans l'interface d'administration de Jeedom.
* Cliquez sur le menu Plugins / Gestion des plugins.
* Cliquez sur Market.
* Recherchez mobile.
...
* Redémarrez le Raspberry Pi.
  ...
  sudo reboot
* Modifiez les configurations du routeur afin que le port externe soit celui entré plus haut (ex : 8088). Le port interne peut demeurer le 80.
...
Le plugin Script permet d'effectuer différentes opérations dans Jeedom et de récupérer des valeurs qui pourront être utilisées dans les scénarios.
