...
Si vous prenez soin d'effectuer régulièrement une sauvegarde de Home Assistant, vous pourrez remettre le système en place rapidement en cas de problème avec votre carte micro SD.

Dans le cas où Home Assistant est encore fonctionnel, vous pouvez procéder directement à l'[étape de restauration](72_sauvegarde_de_home_assistant.md#restauration).

S'il ne fonctionne plus du tout, vous devrez d'abord effectuer une [réinstallation](72_sauvegarde_de_home_assistant.md#reinstallation).

## Réinstallation {#reinstallation}

* Sur votre boîte Home Assistant actuelle, effectuez une sauvegarde complète.
* Téléchargez la sauvegarde sur votre ordinateur.
* Sur la carte micro SD qui contiendra une copie intégrale du système, effectuez une nouvelle installation de Home Assistant.
* Une fois l'installation complétée, accédez à l'interface Web de Home Assistant.
...
* Poursuivez avec [les étapes communes pour la réinstallation et la restauration](72_sauvegarde_de_home_assistant.md#commune).
...
* Pendant la restauration, il est possible de voir l'avancement des travaux dans une fenêtre Terminal à l'aide de la commande ha supervisor logs.
* À la fin de l'opération, vous aurez votre Home Assistant tel qu'il était au moment où vous avez effectué cette sauvegarde.
* Pour vous assurer que tout soit bien réinitialisé, prenez soin de redémarrer le système.
