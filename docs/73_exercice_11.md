...
2. Configurez votre clé Z-Wave sur Home Assistant.
3. Ajoutez un capteur et un récepteur de votre choix à votre système. Rappel : les modes d'emplois de vos objets connectés sont sur Teams.
...
6. Créez une sauvegarde de Home Assistant puis téléchargez-la sur votre poste de travail.
7. Afin de pouvoir travailler sur votre système domotique à la maison, vous avez deux choix. Vous devez sélectionner celui qui vous convient et effectuer les manipulations en conséquence.  
   ...
   2. Connecter le Raspberry Pi à votre réseau sans fil à la maison ou à l'aide du partage de connexion de votre cellulaire. Vous devrez créer un autre fichier de configuration pour ce réseau.
...
9. OPTIONNEL : effectuez ces manipulations pour vous brancher manuellement à un autre réseau sans fil. Dans les manipulations que je vous propose, vous vous brancherez au réseau régulier du Cégep. Par contre, vous ne pourrez pas accéder à l'interface Web de Home Assistant à partir de ce réseau.
   ...
   2. Accédez au terminal HassOS.
   3. Demandez à HassOS de vous lister les réseaux sans fil disponibles.
   4. Repérez le réseau régulier du Cégep. C'est la colonne SSID qui nous intéresse.
   5. Connectez le Pi à ce réseau.
   6. Vérifiez que vous avez maintenant une adresse IP qui correspond à ce réseau.
   7. Reconnectez-vous maintenant à Domotique-Pedago.
   8. Déconnectez maintenant le Wi-Fi. Plusieurs manipulations pourraient être requises pour que tous les réseaux sans fil soient déconnectés. La bannière de Home Assistant devrait maintenant vous donner seulement l'adresse IP du réseau filaire.
10. Éteignez Home Assistant de façon sécuritaire.
