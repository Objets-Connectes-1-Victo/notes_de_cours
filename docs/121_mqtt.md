# 107. MQTT {#chapitre-mqtt}

## 107.1 Le protocole MQTT {#fiche-mqtt}
...
### Caractères génériques {#generiques}
...
## 107.2 La sécurité avec MQTT {#fiche-la_securite_avec_mqtt}
...
### Agent test.mosquitto.org {#mosquittoorg}
...
![Échec de connexion](NotesDeCoursApical-420_3a4_vi_objets_connectes_1_a_2025_files/HomeAssistant-MQTT-EchecDeConnexion.png)

### Agent installé localement {#local}
...
#### Installer un agent après-coup ou reconfigurer l'agent local {#installer}
...
### Agent installé sur un second ordinateur {#ip}
...
## Utilisation d'une connexion sécurisée {#securise}
...
![Écouter un sujet](NotesDeCoursApical-420_3a4_vi_objets_connectes_1_a_2025_files/HomeAssistant-MQTT-EcouterUnSujet.png)
...
Les techniques pour utiliser le client MQTT sont détaillées dans la fiche « [publication_et_abonnement_mqtt_avec_home_assistant](121_mqtt.md#fiche-publication_et_abonnement_mqtt_avec_home_assistant) ».
...
```
  ...
  Dans le fichier automation.yaml ou dans l'interface graphique, lorsque les données sont publiées [au format JSON](127_format_json_dans_un_modele.md#fiche-format_json_dans_un_modele), il ne faut pas entourer le modèle de guillemets ou d'apostrophes (dans cet exemple, il n'y a pas de guillemets alentour de state_attr('domaine.identifiant_objet', 'attribut1')).
...
## Abonnement à un canal {#abonnement}
