...
1. Ajoutez une automatisation dans laquelle le capteur réel de votre choix agit sur le récepteur réel de votre choix.
...
4. Installez l'intégration OpenWeatherMap.
5. Ajoutez une carte météo sur votre tableau de bord.
6. Modifiez une de vos automatisations pour que l'action n'ait lieu que s'il fait AU MOINS 10 degrés celcius (ou autre température de votre choix). Assurez-vous que la carte bouton tienne compte des conditions.
...
10. OPTIONNEL : modifiez le script précédent pour qu'il ajoute le nom de famille à la fin de ce qu'il y a déjà dans le capteur virtuel (vous aurez besoin des modèles pour y parvenir).
11. Créez une nouvelle automatisation qui ferme des stores virtuels dès que le soleil se couche.