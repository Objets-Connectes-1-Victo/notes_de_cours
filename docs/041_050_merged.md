...
## 195 — 5.2 Service OpenWeatherMap
...
## 196 — 5.3 Travailler avec l'intégration OpenWeatherMap
...
Dans ce premier exemple, la condition est définie en passant par Entité / �?tat puis en sélectionnant l'entité sun.sun.
...

## 198 — 6.1 Erreur « Message malformed: Integration '' not found »
...
## 199 — 6.2 Automatisation ne s'enregistre pas
...
7. �?crivez un script qui allume une lumière virtuelle nommée « Lumière salon virtuelle », allume un téléviseur virtuel puis, après 5 secondes (plus facile à tester qu'avec des minutes ;-) ), éteint la lumière du salon virtuelle.
...
9. �?crivez un script qui inscrit votre prénom dans un capteur virtuel textuel, attend qu'un capteur réel envoie un signal de votre choix puis inscrit votre votre nom de famille à la place du prénom.
...

## 201 — 9.1 Les modèles dans Home Assistant
...
* [�?diteur de modèles](999_999_merged.md#editeur)
* [Valeur principale d'un capteur](001_010_merged.md#valeur)
* [Capteur avec attributs](999_999_merged.md#attributs)
* [Retrouver un attribut particulier](999_999_merged.md#particulier)
* [Conditions](#conditions)
* [Travailler avec des valeurs numériques](999_999_merged.md#numerique)
* [Variables dans un modèle](999_999_merged.md#variable)
* [Boucles dans un modèle](999_999_merged.md#boucle)
* [Commentaires dans un modèle](999_999_merged.md#commentaire)
* [Identifiants d'objets non conformes](999_999_merged.md#templatesyntaxerror)
* [Résumé des syntaxes](999_999_merged.md#resume)
...
> Avoid using states.sensor.temperature.state, instead use states('sensor.temperature'). It is strongly advised to use the states(), is\_state(), state\_attr() and is\_state\_attr() as much as possible, to avoid errors and error message when the entity isn�?Tt ready yet (e.g., during Home Assistant startup).
...
| {{ states.weather.forecast\_maison }}  Syntaxe équivalente :  {{ states.weather['forecast\_maison'] }} | Donne la valeur principale d'une entité de même que de tous ses attributs.  �? utiliser seulement dans les outils de développement.  La première syntaxe ne fonctionne pas si [apical\_lien\_interne][qu\_est-ce\_qu\_une\_entite,l'identifiant de l'objet,objet][/apical\_lien\_interne] débute par un chiffre. | <template TemplateState(<state weather.forecast\_maison=partlycloudy; temperature=4.5, dew\_point=3.7, temperature\_unit=°C, humidity=95, cloud\_coverage=71.1, uv\_index=0.1, pressure=1025.2, pressure\_unit=hPa, wind\_bearing=217.2, wind\_speed=9.4, wind\_speed\_unit=km/h, visibility\_unit=km, precipitation\_unit=mm, attribution=Weather forecast from met.no, delivered by the Norwegian Meteorological Institute., friendly\_name=Forecast Maison, supported\_features=3 @ 2025-10-25T08:45:35.357881-04:00>)> |
...
## 202 — 9.2 Exemple d'automatisation avec un modèle
...
## 203 — 9.3 Retrouver les valeurs d'une entité
...
## 204 — 9.4 Quelques manipulations de chaînes dans les modèles Home Assistant
...
* [Longueur d'une chaîne](999_999_merged.md#longueur)
* [Recherche d'une position](999_999_merged.md#position)
* [Sous-chaîne](999_999_merged.md#souschaine)
...
* [Représentation d'une date dans Home Assistant](999_999_merged.md#representation)
* [Date du jour](999_999_merged.md#datedujour)
* [Fuseau horaire](999_999_merged.md#fuseauhoraire)
* [Convertion d'une date en timestamp](021_030_merged.md#conversiondatetimestamp)
  + [timestamp d'une date sous forme de chaîne](999_999_merged.md#chaine)
  + [timestamp d'un objet de type datetime](999_999_merged.md#datetime)
  + [timestamp d'une date codée en dur](999_999_merged.md#dur)
  + [timestamp d'un sensor.date\_time](999_999_merged.md#sensor)
  + [timestamp d'un sensor.time](999_999_merged.md#time)
* [Conversion d'un timestamp en chaîne](999_999_merged.md#conversiontimestampchaine)
* [Calculs avec un timestamp](999_999_merged.md#calculstimestamp)
* [Comparaison avec la date du jour](999_999_merged.md#comparaisondatedujour)
* [Comparaison avec l'heure actuelle sans calculs](999_999_merged.md#comparaisonheureactuelle)
* [Comparaison avec l'heure actuelle si besoin d'effectuer des calculs](999_999_merged.md#comparaisonheureactuelleaveccalculs)
...
D'autres exemples sont donnés [plus bas](999_999_merged.md#calculstimestamp).
...
�? preuve, voici quelques modèles qui effectuent la conversion entre l'heure locale et l'heure UTC. Les résultats obtenus sont affichés plus bas.
...
## 205 — 9.7 Modèles qui vérifient la présence dans une zone
...
## 206 — 9.8 Format JSON dans un modèle
...

�?crivez vos réponses à ces questions dans un fichier texte. Pour les scripts et automatisations, inscrivez le code YAML généré.
...
1. �?crivez un modèle qui retourne la valeur d'un capteur virtuel booléen. Testez ce modèle dans les outils de développement. Modifiez sa valeur dans le tableau de bord et assurez-vous que le modèle reflète le changement.
2. �?crivez un modèle qui retourne true seulement si le capteur virtuel booléen est à true.
3. �?crivez un modèle qui retourne la valeur d'un de vos capteurs réels, par exemple la luminosité d'un capteur 5 en 1.
4. �?crivez un modèle qui retourne  « ouverte » ou « fermée » selon l'état d'un capteur d'ouverture de porte réel ou virtuel.
5. �?crivez un modèle qui retourne la valeur d'un capteur de votre choix multipliée par 5.
6. �?crivez un modèle qui retourne la température en fahrenheit à partir d'un capteur qui donne la température en celcius. Vous pouvez utiliser un capteur réel ou encore le capteur de météo.
7. Créez un capteur virtuel nommé Efforts. Il doit être en mesure de fournir un nombre situé entre 0 et 10. Associez ce capteur à l�?Ticône « brain ».
8. Créez un récepteur virtuel textuel nommé Résultats. Associez ce récepteur à l�?Ticône « chart-bell-curve-cumulative ».
9. Créez une automatisation nommée « Efforts influencent résultats » qui fait en sorte qu�?Tà chaque fois que les efforts changent, les résultats prennent la valeur « Efforts \* 10 » suivi du signe %.
10. �?crivez un modèle qui calcule le nombre de jours restants avant Noël.
...

## 208 — 2.1 Les fichiers journaux de Home Assistant

�? l'aide de l'intégration [System Log](https://www.home-assistant.io/integrations/system_log/), le supervisor de Home Assistant enregistre dans différents fichiers journaux (log files) de l'information sur ce qui se passe dans le système, principalement les erreurs et avertissements.
...
## 209 — 2.2 �?crire dans un fichier journal

Home Assistant propose des services pour écrire dans différents fichiers journaux.

Dans cette fiche :

* [�?criture dans le journal des activités (logbook.log)](001_010_merged.md#logbook)
  + [Identifiant d'entité à référencer](001_010_merged.md#identite)
* [�?criture dans le journal de Home Assistant (system\_log.write)](999_999_merged.md#systemlog)
  + [Niveau de journalisation](001_010_merged.md#niveau)
  + [�?criture](999_999_merged.md#ecriture)
* [Enregistrer l'état d'un capteur dans un fichier journal](001_010_merged.md#etat)

## �?criture dans le journal des activités (logbook.log)
...
## �?criture dans le journal de Home Assistant (system\_log.write)
...
Si vous utilisez l'action �?crire, qui correspond à l'action system\_log.write, vous écrirez un message dans le fichier journal de Home Assistant.
...
�?tat de la lumière : on, modifié par automation.capteur\_virtuel\_agit\_sur\_recepteur\_virtuel
...
![�?crire dans un capteur virtuel](NotesDeCoursApical-420_3a4_vi_objets_connectes_1_a_2025_files/HomeAssistant-DebogageAvecCapteurVirtuelTexte.png)
...

## 210 — 3.1 Afficher la date et l'heure dans le tableau de bord
...
## 211 — 3.2 Automatisation qui tient compte de l'heure
...
Je vous présente ici quelques techniques. �? vous de les tester afin de trouver celle qui répond le mieux à votre besoin.
...
Dans les deux cas, le type de déclencheur sera Entité / �?tat.
...
Entrez ensuite, dans la zone �?, l'heure à laquelle vous désirez que le déclencheur lance l'action.
